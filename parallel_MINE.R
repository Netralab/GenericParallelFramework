...
func_MINE <- function(ds)
{
  col_name_ds = colnames(ds[,-1]) #get column names
  res1 <- mine(ds, y = NULL, master = NULL, alpha = 0.6, C = 1,n.cores = 1, var.thr = 1e-05, eps = NULL, est = "mic_approx",na.rm = FALSE, use = "all.obs", normalization = FALSE)
  result<-as.data.frame(res1[1])
  res <- result[,-1][-1,]
  diag(res)<-0
  thres <- (max(res)-min(result))/2
...
write.table(adj_matrix, file=paste("output folder address","mine_4000", sep = "_"), quote=FALSE, sep='\t')
# done

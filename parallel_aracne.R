...
# 1. ARACNE
func_aracne <- function(ds)
{
  result <- minet( ds, method="aracne", estimator="spearman" )
  res <- result[,-1][-1,]
...
write.table(adj_matrix, file=paste("output folder address","aracne_4000", sep = "_"), quote=FALSE, sep='\t')
# done

  ...
  func_ppcor <- function(ds)
  {
    #ds <- ds[,-1]
    col_name_ds = colnames(ds[,-1]) #get column names
    res1 <- pcor( ds, method="pearson" )
  ...
  write.table(adj_matrix, file=paste("output folder address","ppcor_4000", sep = "_"), quote=FALSE, sep='\t')
  # done

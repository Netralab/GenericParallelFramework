...
# RELNET
func_relnet <- function(ds)
{
  result <- build.mim(ds,estimator="spearman",disc ="none",nbins = sqrt(NROW(dataset)))
  res <- result[,-1][-1,]
...
write.table(adj_matrix, file=paste("output folder address","relnet_4000", sep = "_"), quote=FALSE, sep='\t')
# done

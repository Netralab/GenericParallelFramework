...
#### MutRank
func_mutrank <- function(ds)
{
  m <- cor(ds ,method = "pearson")
  r <- apply(m,1,rank)
  diag(r) <- 0
  net <- r*t(r)/2
  colnames(net) <- colnames(ds)
  rownames(net) <- colnames(ds)
  res <- net[,-1][-1,]
  thres <- ((res[which.max(res)]-res[which.min(res)])/2)
  for(row in 1:nrow(res))
  {
    for(col in 1:ncol(res)) 
    {
      if(res[row, col] > thres) 
      {res[row, col] <- 1}
      else {res[row, col] <- 0}
    }
  }
...
mod_matrix =foreach(i=1:no_of_div)%dopar%{
  ...
  res_matrix = func_mutrank(re_div)
  return(res_matrix) 
}
#done
...
write.table(adj_matrix, file=paste("output folder address","mutrank_4000", sep = "_"), quote=FALSE, sep='\t')
# done

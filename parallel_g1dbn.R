...
col_name_ori = colnames(data_main) #get column names
tot_no_of_col = ncol(data_main) #get number of columns

#data_main = t(data_main)
# done

    ############## G1DBN function #############
    func_g1dbn<-function(ds)
    {
      col_name_ds = colnames(ds) #get column names
      p= ncol(ds)
      n= nrow(ds)
      S1<- DBNScoreStep1(ds, method='ls')
      G1<- BuildEdges(S1$S1ls, threshold=0.5,dec= 'FALSE')
      Step1InferredNet <- BuildNetwork(G1,1:p)
      res<-as.data.frame(Step1InferredNet[3])
      diag(res)<-0
    ...
    # Splitting dataset
    no_of_div = 80 #specify no. of divisions
    col_in_each_div = (tot_no_of_col)/no_of_div #deciding the number of columns in each divisions
    time_col = 1
    count = 1
    ...
    # Re-infering to get final sub-matrices # varies acc to 1inference algorithm
    mod_matrix =foreach(i=1:no_of_div)%dopar%{
      re_div = as.data.frame(mod_div[i])
      res_matrix = func_g1dbn(re_div)
      return(res_matrix) 
    }
    #done
    ...
    write.table(adj_matrix, file=paste("output folder address","g1dbn_2000", sep = "_"), quote=FALSE, sep='\t')
    # done
